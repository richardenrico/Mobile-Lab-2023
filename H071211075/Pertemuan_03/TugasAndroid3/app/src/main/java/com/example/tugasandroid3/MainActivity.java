...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sButton.setOnClickListener(view -> {
            ...
            if(uImage.getDrawable().getConstantState() == getResources().getDrawable(R.drawable.img).getConstantState()) {
                Toast.makeText(this, "Upload image first", Toast.LENGTH_SHORT).show();
            }
            if(!TextUtils.isEmpty(edUser.getText().toString())
                    && !TextUtils.isEmpty(edName.getText().toString())
                    && (uImage.getDrawable().getConstantState() != getResources().getDrawable(R.drawable.img).getConstantState())){
